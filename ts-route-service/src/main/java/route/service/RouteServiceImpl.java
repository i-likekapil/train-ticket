...
import route.controller.RouteController;
...
//            route = routeRepository.findById(info.getId());
//            route = routeRepository.findById(info.getId());
...
//        route = routeRepository.findById(routeId);
    ...
    @Override
    public Response getRouteById(String routeId, HttpHeaders headers) throws Exception {
        if(RouteController.captureVal == 1){
            float curRandomNumber = (float) ((int) (Math.random() * 10000)) / 100;
            if(curRandomNumber <= RouteController.percentageVal){
                Thread.sleep(10000);
                SocketTimeoutException exception = new SocketTimeoutException("Connection Time Out");
//                String stackTrace = String.valueOf(exception.getStackTrace());
                LOGGER.error("stackTrace is......" + Arrays.toString(exception.getStackTrace()));
                throw exception;
            }
        }
        Route route = routeRepository.findById(routeId);
...
//        routes = routeRepository.findAll();
    ...
    @Override
    public Response getAllRoutes(HttpHeaders headers) throws Exception {
        System.out.println("value of capture is: " + RouteController.captureVal);
        if(RouteController.captureVal == 1){
            float curRandomNumber = (float) ((int) (Math.random() * 10000)) / 100;
            if(curRandomNumber <= RouteController.percentageVal){
                Thread.sleep(10000);
